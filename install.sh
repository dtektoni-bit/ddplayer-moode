...
#   ddplayer_dac.c, Makefile, ddplayer-dac.dts
...
sudo apt-get install -y gcc make flex bison libssl-dev bc
...
if [ ! -d "$HEADERS_DIR" ]; then
    ...
    # Extract base version and arch suffix for package name
    # e.g. 6.12.62+rpt-rpi-v8 → linux-headers-6.12.62+rpt-rpi-v8
    PKG_NAME="linux-headers-$KERNEL_VER"
    # Also install common headers package
    # e.g. 6.12.62+rpt-rpi-v8 → linux-headers-6.12.62+rpt-common-rpi
