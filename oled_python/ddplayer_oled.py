...
FONT_SMALL       = 9
FONT_LARGE       = 18
FONT_PATH        = "/usr/share/fonts/truetype/Monocraft.ttf"
...
FILTERS = {(0,0):"SHARP",(1,0):"SLOW",(0,1):"SDSHARP",(1,1):"SDSLOW"}
INPUTS  = {(0,0):"NETWORK",(1,0):"OPTICAL",(0,1):"COAX",(1,1):"NETWORK"}
            ...
            try:
                ...
                if mt != last_spot_meta_mtime:
                    ...
                    if len(parts) >= 4:
                        ...
                        # новый трек — сброс позиции и состояния
                        with state["lock"]:
...
def draw_stop():
    ...
    d.text((0, 0), f"{ip}  {net}", font=fS, fill=1)
    ts = now.strftime("%H:%M:%S")
    d.text((cx(d,ts,fL), 11), ts, font=fL, fill=1)
    ds = now.strftime("%d.%m.%Y")
    d.text((cx(d,ds,fS), 35), ds, font=fS, fill=1)
    d.text((cx(d,filt,fS), 46), filt, font=fS, fill=1)
    inp_str = inp if inp != "NETWORK" else "NETWORK READY"
    d.text((cx(d,inp_str,fS), 55), inp_str, font=fS, fill=1)
...
def draw_play(scroll_x):
    ...
    d.text((0, 0), sr_str, font=fS, fill=1)
    d.text((128 - tw(d,filt,fS), 0), filt, font=fS, fill=1)
    st_str = "PLAY" if st=="play" else "PAUSE"
    fmt = song.get("file","").split(".")[-1].upper()[:4] if "file" in song else ""
    d.text((0, 12), f"{st_str} {fmt}", font=fS, fill=1)
...
def draw_spotify(scroll_x):
    ...
    remaining = max(0, duration - elapsed)
    # Строка 1
    d.text((0, 0), "Spotify", font=fS, fill=1)
    d.text((128 - tw(d,filt,fS), 0), filt, font=fS, fill=1)
    # Строка 2
    st_str = "PLAY" if playing else "PAUSE"
    d.text((0, 12), st_str, font=fS, fill=1)
    t_str = f"{int(remaining)//60}:{int(remaining)%60:02d}"
    d.text((128 - tw(d,t_str,fL), 11), t_str, font=fL, fill=1)
    # Строка 3: artist
    d.text((0, 35), artist[:18], font=fS, fill=1)
